...
describe("UCSBDiningCommonsMenuItemIndexPage tests", () => {
    ...
    test("Renders with Create Button for admin user", async () => {
        ...
        axiosMock.onGet("/api/ucsbmenuitems/all").reply(200, []);
        ...
        expect(button).toHaveAttribute("href", "/ucsbmenuitems/create");
    ...
    test("renders empty table when backend unavailable, user only", async () => {
        ...
        setupUserOnly();
        const queryClient = new QueryClient();
        axiosMock.onGet("/api/ucsbmenuitems/all").timeout();
        ...
        expect(errorMessage).toMatch("Error communicating with backend via GET on /api/ucsbmenuitems/all");
        restoreConsole();
